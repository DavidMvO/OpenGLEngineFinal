...
GPUParticleEmitter::GPUParticleEmitter(GUIBar* gui) : m_particles(nullptr), m_maxParticles(0), m_position(0, 0, 0),
										   m_drawShader(0), m_updateShader(0), m_lastDrawTime(0)
{
		m_vao[0] = 0;
		m_vao[1] = 0;
		m_vbo[0] = 0;
...
void GPUParticleEmitter::Initialise(unsigned int a_maxParticles, float a_lifetimeMin,
									float a_lifetimeMax, float a_velocityMin,
...
{
	unsigned int vs = loadShader(GL_VERTEX_SHADER, "./Res/Shaders/gpuParticle.vert");
...
{
	// create a shader
	unsigned int vs = loadShader(GL_VERTEX_SHADER, "./Res/Shaders/gpuParticleUpdate.vert");
